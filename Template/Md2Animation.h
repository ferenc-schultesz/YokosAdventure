...
public:
	CMD2Model();
	~CMD2Model();
	CMD2Model(const CMD2Model &x);
	bool Load(char *md2fileCharacter, char *imgFileCharacter, char *md2fileWeapon, char *imgFileWeapon);
	bool LoadNoWeapon(char *md2fileCharacter, char *imgFileCharacter);
	void Render(bool renderWeapon);
	void SetAnimation(int i);
	int GetAnimation();
	bool IsLastFrameRendered();
	void SetLastFrameRendered(bool flag);
private:
	t3DModel g1_3DModel;	// Character model
	t3DModel g2_3DModel;	// Weapon model			
	...
	float m_elapsedTime;
	float m_lastTime;
	CHighResolutionTimer m_highResTimer;

	void AnimateMD2Model(t3DModel *pModel, t3DModel *pModel2, bool renderWeapon);
	void Draw(t3DModel *pModel, int currentFrame, int nextFrame, float t);
