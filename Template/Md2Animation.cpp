...
//////////// *** NEW *** ////////// *** NEW *** ///////////// *** NEW *** ////////////////////
	...
	for (int i=0; i < m_Header.numFrames; i++)
	{
		...
		for (int j=0; j < m_Header.numVertices; j++)
		{
			...
			float *n = NormalTable[pFrame->aliasVertices[j].lightNormalIndex];
			pVertices[j].normal[0] = n[0];
			pVertices[j].normal[2] = -n[1];
			pVertices[j].normal[1] = n[2];
		}
	}
}
...
void CLoadMD2::ConvertDataStructures(t3DModel *pModel)
{
	...
	for (i=0; i < pModel->numOfObjects; i++)
	{
		...
		currentFrame.pNormals = new CVector3[currentFrame.numOfVerts];
		...
		for (j=0; j < currentFrame.numOfVerts; j++)
		{
			...
			currentFrame.pNormals[j].x = m_pFrames[i].pVertices[j].normal[0];
			currentFrame.pNormals[j].y = m_pFrames[i].pVertices[j].normal[1];
			currentFrame.pNormals[j].z = m_pFrames[i].pVertices[j].normal[2];
		}
...
float CMD2Model::ReturnCurrentTime(t3DModel *pModel, int nextFrame)
{
	...
	float time = 1000.0f* (float) m_highResTimer.Elapsed();
	...
	glBegin(GL_TRIANGLES);
		...
		for(int j = 0; j < pFrame->numOfFaces; j++)
		{
			...
			for(int whichVertex = 0; whichVertex < 3; whichVertex++)
			{
				...
				CVector3 vNorm;
				vNorm.x = vNorm1.x + t * (vNorm2.x - vNorm1.x);
				vNorm.y = vNorm1.y + t * (vNorm2.y - vNorm1.y);
				vNorm.z = vNorm1.z + t * (vNorm2.z - vNorm1.z);
				float mag = sqrt(vNorm.x*vNorm.x + vNorm.y*vNorm.y + vNorm.z*vNorm.z);
				vNorm.x /= mag;
				vNorm.y /= mag;
				vNorm.z /= mag;
...
{
	if (g1_3DModel.currentAnim == i) { return; }
...
bool CMD2Model::LoadNoWeapon(char *md2fileCharacter, char *imgFileCharacter)
{
	CLoadMD2 g1_LoadMd2;
	CLoadMD2 g2_LoadMd2;
